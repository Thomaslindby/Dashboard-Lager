...
# 3. Omdan til dashboard-format
data = []
for row in rows:
    keys = list(row.keys())
    name_col  = next((k for k in keys if "medarbej" in k.lower() or "lager" in k.lower()), keys[0])
    pluk_col  = next((k for k in keys if "antal" in k.lower() and "pluk" in k.lower()), keys[1] if len(keys)>1 else keys[0])
